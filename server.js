...
Always stay conversational and clear. You are not a search engine or a robot — you are Sage.`.trim()
      ...
      const isGroqRelevant = groqAnswer && !irrelevantResponses.some(msg =>
        groqAnswer.toLowerCase().includes(msg.toLowerCase())
      );

      if (isGroqRelevant) return res.json({ answer: groqAnswer });

      const searchResults = await performWebSearch(query);
      return res.json({ answer: `I couldn't find a specific answer in the school's data, but here’s what I found online:\n\n${searchResults}` });
